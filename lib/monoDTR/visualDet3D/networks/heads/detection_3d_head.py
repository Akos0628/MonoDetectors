...
from lib.monoDTR.visualDet3D.networks.heads.losses import SigmoidFocalLoss, ModifiedSmoothL1Loss
from lib.monoDTR.visualDet3D.networks.heads.anchors import Anchors
from lib.monoDTR.visualDet3D.networks.utils.utils import calc_iou, BackProjection
from lib.monoDTR.visualDet3D.networks.lib.fast_utils.hill_climbing import post_opt
from lib.monoDTR.visualDet3D.networks.utils.utils import ClipBoxes
from lib.monoDTR.visualDet3D.networks.lib.blocks import AnchorFlatten
...
class AnchorBasedDetection3DHead(nn.Module):
    def __init__(self, head_cfg):
        ...
        anchors_cfg = head_cfg['anchors_cfg']

        self.anchors = Anchors(
            anchors_cfg['preprocessed_path'],
            anchors_cfg['pyramid_levels'],
            anchors_cfg['strides'],
            anchors_cfg['sizes'],
            anchors_cfg['ratios'],
            anchors_cfg['scales'],
        ...
        self.build_loss(
            loss_cfg['focal_loss_gamma'], 
            loss_cfg['balance_weight'], 
            loss_cfg['L1_regression_alpha'], 
            loss_cfg['regression_weight']
        )
        ...
        self.init_layers(
            layer_cfg['num_features_in'],
            layer_cfg['num_anchors'],
            num_classes+1,
            layer_cfg['num_reg_output'],
            layer_cfg['cls_feature_size'],
            layer_cfg['reg_feature_size']
        )
